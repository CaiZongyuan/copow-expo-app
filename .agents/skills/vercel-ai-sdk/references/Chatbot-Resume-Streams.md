...
When you enable `resume`, the `useChat` hook makes a `GET` request to `/api/v1/chat/[id]/stream` on mount to check for and resume any active streams.
...
```ts filename="app/api/v1/chat/route.ts"
...
Create a GET handler at `/api/v1/chat/[id]/stream` that:
...
```ts filename="app/api/v1/chat/[id]/stream/route.ts"
...
3. **Client reconnection**: When the client reconnects (page reload), the `resume` option triggers a GET request to `/api/v1/chat/[id]/stream`
...
By default, the `useChat` hook makes a GET request to `/api/v1/chat/[id]/stream` when resuming. Customize this endpoint, credentials, and headers, using the `prepareReconnectToStreamRequest` option in `DefaultChatTransport`:
  ...
  const { messages, sendMessage } = useChat({
    ...
    transport: new DefaultChatTransport({
      ...
      prepareReconnectToStreamRequest: ({ id }) => {
        return {
          api: `/api/v1/chat/${id}/stream`, // Default pattern
