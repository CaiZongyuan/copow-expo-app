...
3. **Create Issue:**
   ...
   **Note**: Use `--body-file -` instead of `--body` to avoid bash parsing errors from backticks in markdown code blocks.
...
## Important: Use `--body-file -` for Complex Content

**Always use `--body-file -` with heredoc instead of `--body "$(cat ...)"`**

**Why**: When issue body contains markdown code blocks with backticks (`` ``` ``), using `--body "$(cat <<'EOF'...EOF)"` causes bash parsing errors because the command substitution `$(...)` scans for unescaped backticks.
...
**Incorrect Pattern** (causes syntax errors):
```bash
gh issue create --title "Title" \
  --body "$(cat <<'EOF'
## Issue Body
