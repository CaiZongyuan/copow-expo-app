...
By default, the `useChat` hook sends a HTTP POST request to the `/api/v1/chat` endpoint with the message list as the request body. You can customize the request in two ways:
...
```ts filename="app/api/v1/chat/route.ts" highlight="3,4"
export async function POST(req: Request) {
...
```ts filename="app/api/v1/chat/route.ts" highlight="13-27"
import { convertToModelMessages, streamText, UIMessage } from "ai";
__PROVIDER_IMPORT__;
...
import { useChat } from "@ai-sdk/react";
import type { MyUIMessage } from "./api/v1/chat/route";
import { DefaultChatTransport } from "ai";
...
```ts filename="app/api/v1/chat/route.ts" highlight="13"
import { convertToModelMessages, streamText, UIMessage } from "ai";
